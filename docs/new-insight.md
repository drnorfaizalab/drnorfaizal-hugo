...
## SEO / GEO — built into every post

Every post generated by this pipeline now automatically ships with:

- **FAQ section** — 4-5 Q&A pairs written for the topic, as both a `faq_schema` front
  matter field (drives Google FAQ rich results) and a matching visible "Frequently
  asked questions" / "Soalan lazim" section in the body. This is also what gets quoted
  directly by AI answer engines (ChatGPT, Gemini) when someone asks a related question.
- **Video schema** — if `raw_thoughts` includes a YouTube link (including `/shorts/`
  links), Gemini extracts the video ID, embeds it with the `{{< youtube "ID" >}}`
  shortcode, and adds `youtube_id: "ID"` to the front matter so the video gets its own
  VideoObject rich result.
- **Title length control** — if the natural title runs long, a shorter `seoTitle` is
  added alongside it so the `<title>` tag and search snippet stay under ~60 chars
  without changing the on-page heading.

You don't need to ask for any of this — just write `raw_thoughts` normally and paste
the YouTube link inline if there is one.
