---
title: "Neurooncology Surgery: Brain & Spinal Tumour Specialist"
description: "Advanced brain & spinal tumour surgery. Meningioma, glioma, medulloblastoma specialist. Precision neurooncology with minimally invasive techniques."
date: 2025-03-10
lastmod: 2025-03-10
draft: false

type: "specialty"
weight: 1

# SEO
keywords: "neurooncology, brain tumour surgery, meningioma, glioma, medulloblastoma, spinal tumour, Malaysia, neurosurgeon"

# Content
image: "/images/neurooncology-hero.jpg"
sections:
  - type: "hero"
    title: "Advanced Brain & Spinal Tumour Surgery"
    subtitle: "Precision neurooncology for meningioma, glioma, medulloblastoma, and more"
  - type: "cta"
    button_text: "Schedule Consultation"
    button_link: "https://www.kpjhealth.com.my/dr-nor-faizal-ahmad-bahuri"

---

## What is Neurooncology?

Neurooncology is the surgical specialty focused on diagnosing and treating tumours of the brain and spinal cord. As an Oxford-trained neurosurgeon, I combine cutting-edge surgical techniques with precision-driven care to achieve the best possible outcomes for patients with complex neurological tumours.

## Brain Tumours We Treat

### Meningioma
Benign or malignant tumours arising from the meninges (brain lining).
- **Symptoms:** Headaches, vision changes, hearing loss
- **Approach:** Gross total resection when safely possible
- **Outcome:** Generally excellent prognosis with surgery

### Glioma & Glioblastoma
Tumours arising from glial cells within the brain.
- **Grade System:** Low-grade (I-II) vs High-grade (III-IV)
- **Treatment:** Surgery + Radiation ± Chemotherapy
- **Surgical Goal:** Maximum resection while preserving function

### Medulloblastoma
Most common malignant paediatric brain tumour.
- **Presentation:** Headache, vomiting, balance problems
- **Multimodal Treatment:** Surgery + Chemotherapy + Radiation
- **Prognosis:** Improved with modern protocols

### Central Neurocytoma
Rare benign tumour of the lateral ventricles.
- **Approach:** Endoscopic resection when possible
- **Outcomes:** Generally excellent

### Brainstem Gliomas
Challenging location; require specialized approach.
- **Technique:** Frameless stereotactic navigation
- **Goal:** Safe debulking while preserving brainstem function

## Surgical Techniques

### Awake Craniotomy
For tumours near eloquent cortex (speech, motor, vision areas):
- Patient remains conscious during surgery
- Real-time neurological monitoring
- Preserves brain function while removing tumour
- Allows maximum resection safely

### Frameless Stereotactic Navigation
3D imaging-guided surgery for precision:
- Real-time tracking of surgical instruments
- Reduced invasiveness
- Better outcomes for complex cases

### Neuromonitoring
Continuous monitoring during surgery:
- Electromyography (EMG)
- Motor evoked potentials (MEP)
- Somatosensory evoked potentials (SSEP)
- Motor pathway mapping
- Ensures functional preservation

## Symptoms That Require Evaluation

- **Persistent or worsening headaches** (especially new pattern)
- **Vision changes** (blurring, visual field loss)
- **Balance problems** or vertigo
- **Weakness or numbness**
- **Speech or language difficulties**
- **Seizures**
- **Cognitive changes** or memory loss
- **Unexplained nausea/vomiting**

If you experience these symptoms, seek neurological evaluation promptly.

## Diagnostic Workup

**MRI of the Brain** (gold standard)
- High-resolution imaging of tumour location, size, and extent
- Helps guide surgical planning

**CT Scan** (for bone detail if needed)

**PET/SPECT** (for grading and prognosis)

**Molecular Testing** (for grade and prognosis)

## Treatment Goals

1. **Diagnosis Confirmation** - Biopsy or resection for pathology
2. **Maximum Safe Resection** - Remove as much tumour as possible while preserving function
3. **Symptom Relief** - Address pressure effects, reduce edema
4. **Functional Preservation** - Maintain neurological function
5. **Prognosis Optimization** - Set stage for adjuvant therapy if needed

## Recovery Timeline

**Hospital Stay:** 2-4 days post-op

**Return to Light Activities:** 2-4 weeks

**Full Recovery:** 6-12 weeks

**Full Neurological Recovery:** 3-6 months (sometimes longer)

**Follow-up:** Regular imaging and neurosurgical follow-up essential for detecting recurrence

## Prognosis by Tumour Type

| Tumour | Grade | 5-Year Survival | Factors |
|--------|-------|-----------------|---------|
| Meningioma | I | >90% | Age, gross total resection, location |
| Glioblastoma | IV | 15-20% | Age, performance status, MGMT status |
| Low-grade Glioma | I-II | 60-80% | Grade, molecular subtype, extent of resection |
| Medulloblastoma | III-IV | 70-80% | Age, stage, molecular subtype |

*Individual prognosis varies based on patient factors, tumour characteristics, and treatment response.*

## Common Questions

**Q: Do all brain tumours need surgery?**  
A: Not necessarily. Treatment depends on tumour type, grade, location, and patient factors. Some may be monitored, others require surgery, radiation, chemotherapy, or combinations.

**Q: Will surgery affect my thinking or personality?**  
A: Neurosurgical techniques are designed to minimize functional impact. Neuromonitoring during surgery helps preserve brain function. Some patients experience temporary confusion post-op, but most recover fully.

**Q: How long is the hospital stay?**  
A: Typically 2-4 days. Early mobilization is encouraged to prevent complications.

**Q: What about chemotherapy and radiation after surgery?**  
A: Depends on tumour type and grade. Your oncology and radiation oncology teams will discuss this based on pathology.

**Q: Can brain tumours come back?**  
A: Some tumours have higher recurrence risk than others. Regular imaging follow-up is essential to detect early recurrence.

---

## Next Steps

If you've been diagnosed with a brain or spinal tumour, or suspect you may have symptoms warranting investigation:

**[Schedule Your Evaluation →](https://www.kpjhealth.com.my/dr-nor-faizal-ahmad-bahuri)**

I'll conduct a comprehensive neurological assessment, review your imaging, and discuss treatment options tailored to your specific case.

---

**Discipline · Simplicity · Elegance**

*Precision neurosurgery for complex tumours. Outcomes that matter.*
